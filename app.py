...
def messages_inbox():
    me = current_user()
    users = query_all("SELECT id, full_name, avatar_path, role FROM users WHERE id != ? AND approval_status='approved' ORDER BY full_name", (me["id"],))
    peer_id = request.args.get("peer", type=int)
    conversation = []
    peer = None
    if peer_id:
        peer = query_one("SELECT id, full_name, avatar_path FROM users WHERE id=?", (peer_id,))
        if peer:
            conversation = query_all(
                """
            ...
            get_db().execute("UPDATE direct_messages SET is_read=1 WHERE sender_id=? AND receiver_id=?", (peer_id, me["id"]))
            get_db().commit()
    return render_template("messages.html", users=users, conversation=conversation, peer=peer, me=me)
...
def messages_send():
    ...
    if text:
        get_db().execute(
            ...
            (me["id"], peer_id, text, now_iso()),
        )
        get_db().commit()
    return redirect(url_for("messages_inbox", peer=peer_id))
